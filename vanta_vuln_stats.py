...
class VantaAPIClient:
    ...
    def get_vulnerabilities(self, page_size: int = 100, **filters) -> List[Dict]:
        """
  ...
  python vanta_vuln_stats.py --output stats.json
    ...
    try:
        ...
        if needs_sync:
            ...
            if args.verbose:
                print("Fetching vulnerabilities from API...")
            vulnerabilities = client.get_vulnerabilities()

            if args.verbose:
                print(f"Fetched {len(vulnerabilities)} total vulnerabilities")

            # Store in database
            if args.verbose:
                print("Storing vulnerabilities in database...")
            sync_stats = db.store_vulnerabilities(vulnerabilities)
            ...
            if args.verbose or args.sync:
                ...
                print(f"  Remediated: {sync_stats['remediated']}")
                print()
        else:
            # Use cached data
            if args.verbose:
